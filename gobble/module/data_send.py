...
class NaverDataSend:
    def make_checklist(self, new_type, date, value):
        if PRODUCTION == True:
    ...
    def data_send(self, func, new_type):
        if new_type == "major":
            nnc = NaverMajorCrawler()
            checklist = self.make_checklist('M', TODAY, 'url')
        elif new_type == "rt":
            nnc = NaverRealtimeCrawler()
            checklist = self.make_checklist('R', TODAY, 'url')
        else:
        ...
        if len(nnc_data) == 0:
            ...
            for nnc_part in nnc_data:
                ...
                if new_type == "major":
                    ...
                    print("major와 rt 중에 선택하시오.")
                if PRODUCTION == True:
                    naver_content_orm.save(using='contents')
                else:
